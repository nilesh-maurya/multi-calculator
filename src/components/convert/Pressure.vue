<template>
  <div class="pressure wrapper-convert-element">
...
export default {
  ...
  data() {
    return {
      toggleFocus: true,
      select1: { unit: "Pascal", abbr: "Pa" },
      select2: { unit: "Standard atmosphere", abbr: "atm" },
      items: [
        { unit: "Pascal", abbr: "Pa" },
        { unit: "Kilopascal", abbr: "kPa" },
        { unit: "Megapascal", abbr: "MPa" },
        { unit: "Hectopascal", abbr: "hPa" },
        { unit: "Bar", abbr: "bar" },
        { unit: "torr", abbr: "torr" },
        { unit: "Standard atmosphere", abbr: "atm" }
      ]
    };
  },
  computed: {
  ...
  methods: {
    ...
    calculatePressure(id) {
      // find conversion here
      let convertedValue;
      if (id === "1") {
        convertedValue = convert(this.first_input, "pressure")
          .from(this.select1.abbr)
          .to(this.select2.abbr);

        mutations.setSecondInput(convertedValue.toString(10));
      } else if (id === "2") {
        convertedValue = convert(this.second_input, "pressure")
